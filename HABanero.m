...
% It also exports all events to three CSV files: Inital before removing tags,
% a collection of matlab tags, and a post-prossessing event list
...
habituation_markers = {'pbeh', 'nonp', 'pcry', 'pvoc'}; 

% Define the markers indicating din events
din_markers = {'DI10', 'DI20', 'DI30'};
...
for idx = length(EEG.event):-1:1  % Iterate backwards to safely remove elements
    ...
    if strcmp(event_type, 'DI10')
        EEG.event(idx).type = 'Firt';
        EEG.event(idx).code = 'Firt';
    elseif strcmp(event_type, 'DI20')
        EEG.event(idx).type = 'Secd';
        EEG.event(idx).code = 'Secd';
    elseif strcmp(event_type, 'DI30')
        EEG.event(idx).type = 'Thid';
        EEG.event(idx).code = 'Thid';
    elseif ~any(strcmp(event_type, [habituation_markers, din_markers]))
        % Remove any event that isn't a 'din' or a habituation marker
        EEG.event(idx) = [];  % Remove the event
    end
end

% 1.5 pass: Creates the intial event tags before removal of the
% habituation markers in second pass

% ============ EEG file name; added by Winko ===============
file_name = app.list_eeg_files.Value; % Get the file name from the app
% ==========================================================

% Extract the ID from the file name (assuming different formats for ID structure)
[~, name, ~] = fileparts(file_name); % Get the file name without extension

% Check if the filename contains an underscore or hyphen
if contains(name, '_') 
...
elseif contains(name, '-')
    id_parts = split(name, '-'); % Split the file name by hyphen
    id = id_parts{1}; % Take the first part as the ID
else
    error('Filename format not recognized.'); % Handle unexpected formats
end
...
initial_out_file = fullfile(out_path, sprintf('%s_hab_initial_events.csv', id));
...
for idx = 2:length(EEG.event)-2  % Adjust loop bounds to prevent out-of-bounds errors
    if strcmp(EEG.event(idx-1).type, 'Firt') && ...
       any(strcmp(EEG.event(idx).type, habituation_markers)) && ...
       strcmp(EEG.event(idx+1).type, 'Secd') && ...
       strcmp(EEG.event(idx+2).type, 'Thid')
        % Mark all events in this sequence for removal
        EEG.event(idx-1).type = 'removed';
        EEG.event(idx).type = 'removed';
        EEG.event(idx+1).type = 'removed';
        EEG.event(idx+2).type = 'removed';
    end
end
...
% Fourth pass: Mark remaining events for the simplest sequence
for idx = 2:length(EEG.event)-1  % Adjust loop bounds to prevent out-of-bounds errors
    if strcmp(EEG.event(idx-1).type, 'Firt') && ...
       any(strcmp(EEG.event(idx).type, habituation_markers))
        % Mark these events for removal
        EEG.event(idx-1).type = 'removed';
...
% Define output file
out_file = fullfile(out_path, sprintf('%s_hab_matlab_events.csv', id));
...
input_csv = fullfile(out_path, sprintf('%s_hab_matlab_events.csv', id)); % Path to the existing CSV file
output_csv = fullfile(out_path, sprintf('%s_hab_final_events.csv', id)); % Path for the new CSV file

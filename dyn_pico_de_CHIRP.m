...
i% === Check for 'seg_str' and 'seg_end' ===
seg_start = 'seg_str';
seg_end = 'seg_end';
...
% Prompt user for DIN markers
din1 = input('Enter first DIN marker: ', 's');
din2 = input('Enter second DIN marker: ', 's');

din_markers = {din1, din2};
...
for idx = length(EEG.event):-1:1  % Iterate backwards to safely remove elements
    ...
    if strcmp(event_type, din_markers{1})
        EEG.event(idx).type = 'Start';
    elseif strcmp(event_type, din_markers{2})
